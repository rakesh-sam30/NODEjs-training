...
const createBulkDownloadQueueStateSlice = (set, get) => (Object.assign(Object.assign({}, exports.initialBulkDownloadQueueState), { addToBulkDownloadQueue: (entry) => {
        const store = get();
        ...
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, i) => {
                ...
                return Object.assign(Object.assign({}, item), { status: download_statuses_1.DownloadStatus.PROCESSING });
            }),
        }));
        ...
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, i) => {
                ...
                return Object.assign(Object.assign({}, item), { filename,
                    total, status: download_statuses_1.DownloadStatus.DOWNLOADING });
            }),
        }));
        ...
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, i) => {
                ...
                return Object.assign(Object.assign({}, item), { filename,
                    total, progress: (item.progress || 0) + chunk.length });
            }),
        }));
        ...
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, i) => {
                ...
                return Object.assign(Object.assign({}, item), { status: download_statuses_1.DownloadStatus.DOWNLOADED });
            }),
        }));
        set((prev) => ({
        ...
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, i) => {
                ...
                return Object.assign(Object.assign({}, item), { status: download_statuses_1.DownloadStatus.FAILED });
            }),
        }));
        set((prev) => ({
            failedBulkDownloadItemCount: prev.failedBulkDownloadItemCount + 1,
        }));
    }, operateBulkDownloadQueue: () => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        ...
        for (let i = 0; i < bulkDownloadQueue.length; i++) {
            ...
            const searchPageDocument = yield (0, utils_1.attempt)(() => (0, document_1.getDocument)(md5SearchUrl));
            if (!searchPageDocument) {
                get().setWarningMessage(`Couldn't fetch the search page for ${item.md5}`);
                get().onBulkQueueItemFail(i);
                continue;
            }
            const entry = (_a = (0, search_1.parseEntries)(searchPageDocument)) === null || _a === void 0 ? void 0 : _a[0];
            if (!entry) {
        ...
        const completedMD5List = get()
            ...
            .map((item) => item.md5);
        try {
            const filename = yield (0, file_1.createMD5ListFile)(completedMD5List);
            set({
    ...
    }), startBulkDownload: () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const md5ListResponse = yield (0, utils_1.attempt)(() => (0, node_fetch_1.default)(findMD5SearchUrl));
        if (!md5ListResponse) {
            get().setWarningMessage("Couldn't fetch the MD5 list");
            return;
        }
        const md5Arr = (yield md5ListResponse.json());
        const md5List = md5Arr.map((item) => item.md5);
        set((prev) => ({
            bulkDownloadQueue: prev.bulkDownloadQueue.map((item, index) => (Object.assign(Object.assign({}, item), { status: download_statuses_1.DownloadStatus.IN_QUEUE, md5: md5List[index] }))),
        }));
        get().operateBulkDownloadQueue();
    }), startBulkDownloadInCLI: (md5List) => __awaiter(void 0, void 0, void 0, function* () {
        set({
        ...
        yield get().operateBulkDownloadQueue();
        // process exit successfully
        get().handleExit();
    }), resetBulkDownloadQueue: () => {
        set(Object.assign({}, exports.initialBulkDownloadQueueState));
    } }));
